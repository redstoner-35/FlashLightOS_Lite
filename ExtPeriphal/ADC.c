...
bool ADC_GetResult(ADCOutTypeDef *ADCOut)
  ...
  float buf,Rt;
    ...
    ADC_RegularChannelConfig(HT_ADC0, _MOS_NTC_Ch, MOS_NTC_Ch, 0);	
		...
		ADC_RegularChannelConfig(HT_ADC0, _IMON_Ch, IMON_Ch, 0);	
  ...
  buf/=(float)VbattDownResK/((float)VbattUpResK+(float)VbattDownResK); //根据电阻分压公式换算实际电池电压
	...
	//计算极亮直驱的LED平均电流
	buf=(float)ADCResult[IMON_Ch]*(ADC_AVRef/(float)4096);//将AD值转换为电压	
	buf*=1000/IMONAmpGain; //将电压转换为mV，并除以电流检测放大器的增益得到实际检流电阻两端的电压
	ADCOut->DDLEDCurrent=buf/DDShuntValue; //除以检流电阻的阻值(mR)得到实际输出电流		
	//计算完毕
	return true;
	}
//内部ADC初始化
void InternalADC_Init(void)
  {
	 ...
	 AFIO_GPxConfig(GPIO_PA,IMON_IOP,AFIO_FUN_ADC0);
   //设置转换组别类型，转换时间，转换模式      
